"""SU2 CFD provider — CPU-only fallback.

Same pipeline as ``su2_cuda`` but forces ``use_gpu=False``.
"""
...
class SU2CpuProvider:
    """SU2 CFD without GPU — runs on CPU only."""

    provider_id: str = "su2_cpu"
    display_name: str = "SU2 (CPU Only)"
...
_instance = SU2CpuProvider()
ProviderRegistry.register(
    CFDProvider,
    _instance,
    ProviderInfo(
        provider_id="su2_cpu",
        display_name="SU2 (CPU Only)",
        protocol_type=CFDProvider,
        requires_software=["SU2_CFD", "gmsh"],
        description="SU2 CFD solver running on CPU only (no GPU required).",
    ),
)
